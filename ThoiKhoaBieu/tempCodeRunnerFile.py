...
class App(tk.Tk):
    def	__init__(self):
        ...
        tphong_file_btn = Button(self, text="File", command=tphong_handle_file_input, width=20)
        ...
        phong24_result_text = Text(self, height=2, width=60)
        phong24_result_text.grid(row=3, column=0, columnspan=3, pady=10)

        tphong_read_btn = Button(self, text="Sắp xếp", command=tphong_handle_SapXep, width=20)
        tphong_read_btn.grid(row=4, column=0,columnspan=3, pady=10)

        #Choice 
        selected_choice = StringVar(self)
        selected_choice.set("Giáo Viên")  # Đặt giá trị mặc định là "Choice 1"
        selected_choice.trace("w", on_select_choice)

        option_choice = StringVar(self)

        radio_button1 = Radiobutton(self, text="Giáo Viên", variable=selected_choice, value="Giáo Viên")
        radio_button1.grid(row=5, column=0,columnspan=2, pady=10)

        radio_button2 = Radiobutton(self, text="Lớp", variable=selected_choice, value="Lớp")
        ...
        tphong_read_btn = Button(self, text="In", command=tphong_handle_In, width=20)
        ...
        tree = ttk.Treeview(self, columns=('Giờ', 'T2', 'T3', 'T4', 'T5', 'T6', 'T7'), show='headings')
