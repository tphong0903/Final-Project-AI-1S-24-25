...
if st.session_state["flag_anim"] == False:
    ...
    algorithm = st.selectbox('Select Algorithm', ['A*', 'BFS', 'DFS'])
    ...
    if st.button('Direction'):
        ...
        if algorithm == 'A*':
            result = astar_search(romania_problem)
        elif algorithm == 'BFS':
            result = best_first_graph_search(romania_problem)
        elif algorithm == 'DFS':
            result = depth_first_graph_search(romania_problem)

        lst_path = result.path()
